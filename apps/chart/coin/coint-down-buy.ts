...
interface CoinConfig {
  ...
  buyQuantity: number;
  sellQuantity: number;
  consecutiveLowBuyThreshold: number;
  consecutiveHighSellThreshold: number;
  consecutiveLowSellThreshold: number;
}
...
const CONSECUTIVE_HIGH_SELL_THRESHOLD = 5;
const CONSECUTIVE_LOW_BUY_THRESHOLD = 8;
const CONSECUTIVE_LOW_SELL_THRESHOLD = 80;

const coinConfigs: { [key: string]: CoinConfig } = {
  'bitcoin': { symbol: 'bitcoin', period: PERIOD, commissionRate: COMMISSION_RATE, buyQuantity: BUY_QUANTITY, sellQuantity: SELL_QUANTITY, consecutiveLowBuyThreshold: CONSECUTIVE_LOW_BUY_THRESHOLD, consecutiveHighSellThreshold: CONSECUTIVE_HIGH_SELL_THRESHOLD, consecutiveLowSellThreshold: CONSECUTIVE_LOW_SELL_THRESHOLD },
  'ethereum': { symbol: 'ethereum', period: PERIOD, commissionRate: COMMISSION_RATE, buyQuantity: BUY_QUANTITY, sellQuantity: SELL_QUANTITY, consecutiveLowBuyThreshold: CONSECUTIVE_LOW_BUY_THRESHOLD, consecutiveHighSellThreshold: CONSECUTIVE_HIGH_SELL_THRESHOLD, consecutiveLowSellThreshold: CONSECUTIVE_LOW_SELL_THRESHOLD },
  'ripple': { symbol: 'ripple', period: PERIOD, commissionRate: COMMISSION_RATE, buyQuantity: BUY_QUANTITY, sellQuantity: SELL_QUANTITY, consecutiveLowBuyThreshold: CONSECUTIVE_LOW_BUY_THRESHOLD, consecutiveHighSellThreshold: CONSECUTIVE_HIGH_SELL_THRESHOLD, consecutiveLowSellThreshold: CONSECUTIVE_LOW_SELL_THRESHOLD },
};
...
async function main(config: CoinConfig) {
  ...
  for (let i = 0; i < coinData.length; i++) {
    ...
    if (previousCoin) {
      ...
      let action = '-';
      if (startIndex === undefined && low === config.consecutiveLowBuyThreshold) {
        startIndex = i;
        buyActionCount++;
      ...
      if (high > config.consecutiveHighSellThreshold) {
      ...
      if (low === config.consecutiveLowSellThreshold) {
        if (startIndex !== undefined && startIndex > 0 && coinCount > 0) {
          sellActionCount++;
          const sellQuantity = Math.min(coinCount, config.sellQuantity);
          sellCount += sellQuantity;
